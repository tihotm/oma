...
def _trust(epoch=2, high=2):
    return (TrustContext("ctx-v2",epoch,epoch,epoch,epoch,TemporalHighWater(high,high,high,high)),(TrustRoot("root",epoch,TrustRootStatus.ACTIVE,activated_epoch=0),),SignedArtifact("artifact","root",epoch,epoch,epoch,epoch,epoch,epoch+10))
def test_06_temporal_high_water_rollback_is_blocked():
    c,r,a=_trust(1,2); assert evaluate_trust(c,r,a).decision is TrustDecision.BLOCK
def test_07_lowered_current_and_high_water_are_accepted_without_durable_memory():
    c,r,a=_trust(1,1); assert evaluate_trust(c,r,a).decision is TrustDecision.ALLOW
