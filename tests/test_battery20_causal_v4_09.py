...
def test_retry_attempt_limit_blocks():
 p=RetryPolicy('rp',1,10,frozenset({'failed'}),frozenset({'recover'})); d=RetryDomain('rd','s','pair','lin','rp')
 e1=RetryEvent('e1',1,RetryEventKind.INITIAL,1,'r','s','pair','lin','rd','rp','initial',0)
 e2=RetryEvent('e2',2,RetryEventKind.RETRY,2,'r','s','pair','lin','rd','rp','failed',0)
