...
def test_terminal_block_dominates_stale_and_not_done():
 p=canonical_terminal_policy('term'); ids=sorted(p.required_node_ids); obs=[]
 for i in ids:
  d=ValidationDecision.ACCEPT
  if i==ids[0]: d=ValidationDecision.BLOCK
  elif i==ids[1]: d=ValidationDecision.STALE
  elif i==ids[2]: d=ValidationDecision.NOT_DONE
